...
private:
    ...
    /**  @brief C++ implementation of defining BEV transformation points
         @description Vertices for the BEV transformation are hard-coded in this function.
    ...
    /**  @brief C++ implementation of Bird's Eye View (BEV) Transformation
         @description Gets the BEV transformation points, calculates the homography matrix and performs the BEV transformation
    ...
    /**  @brief C++ implementation of (Reverse) Bird's Eye View (BEV) Transformation
         @description Gets the BEV transformation points, calculates the reverse homography matrix and performs the BEV transformation
    ...
    /**  @brief C++ implementation of Sliding window lane tracking
         @description Find the lane in the image
         @param[in] 	src            Source image matrix
...
public:
    ...
    /**  @brief C++ implementation of lane detection pipeline
         @description Scales the image, does BEV transformation, Gray-scale conversion, White masking, Image thresholding, Sliding window lane tracking and lateral deviation calculation. Images are also stored in lane_out_img_dir, if RE_DRAW_IMAGE is defined.
         @param[in] src            Source image Matrix
