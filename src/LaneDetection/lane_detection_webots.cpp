...
long double laneDetectionWEBOTS::lane_detection_pipeline(Mat src_in, int world_encode, int pipe){
	...
	bool condition = (world_encode == 7);
	if (condition) {
		vector<Mat> splited_frame;
		Mat merged;
		split(img_warped, splited_frame);
		for (unsigned int i = 0; i < splited_frame.size(); i++)
			threshold(splited_frame[i], splited_frame[i], threshold_value, max_value, THRESH_BINARY | THRESH_OTSU);
		
		merged = splited_frame[0].clone();
		//for(unsigned int i = 1; i < splited_frame.size(); i++) merged &= splited_frame[i];
		bitwise_not(merged, img_lanes);
	}
	
	condition = (world_encode == 5) && (pipe == 2 || pipe == 3 || pipe == 8);
	if (condition) {
		vector<Mat> splited_frame;
		Mat merged;
		split(img_warped, splited_frame);
	...
	condition = (world_encode == 4) && (pipe == 8);
	if (condition) {
		vector<Mat> splited_frame;
		Mat merged;
		split(img_warped, splited_frame);
	...
	#ifdef DEBUG
		cout << "[IMACS DEBUG] No. of lane-pixels: " << lanes[0].size() << " and " << lanes[1].size() << endl; 
	#endif	
	...
	long double yL = calculate_lateral_deviation(lanes[0], lanes[1]);
		...
		lane_identification(lanes, src, img_roi, img_warped, img_lanes, img_detected_lanes, draw_lines, diff_src_rebev, img_rev_warped, world_encode);
	...
	#ifdef DEBUG
		cout << "[IMACS DEBUG] Lateral_deviation : " << yL << " m" << endl;
	#endif
